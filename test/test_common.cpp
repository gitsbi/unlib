...
SUBCASE("literal operator result types") {
	...
	const auto kWi = 42_kW;
	using kWi_t = decltype(kWi);
	const auto kWf = 42._kW;
	using kWf_t = decltype(kWf);
	SUBCASE("default") {
		CHECK( typeid(kWi_t::value_type) == typeid(long) );
		CHECK( typeid(kWf_t::value_type) == typeid(double) );
	}
}
...
SUBCASE("mass") {
	...
	CHECK( typeid(1_kt) == typeid(unlib::kilo <unlib:: ton<integer_value_type>>) ); CHECK( 1_kt == unlib::kilo <unlib:: ton<integer_value_type>>{1} );
